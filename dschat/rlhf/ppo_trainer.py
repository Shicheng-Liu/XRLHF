...
class DeepSpeedPPOTrainer():
    ...
    def __init__(self, rlhf_engine, args):
        ...
        self.critic_model = self.critic_model.to(torch.bfloat16)
    ...
    def train_rlhf(self, inputs):
        ...
        #        print(f"[Inf] in {name}")
        #    if param.abs().max() > 1e4:  # heuristic threshold
        ...
        #        print(f"{k}: NaNs={torch.isnan(v).any()}, max={v.max()}, min={v.min()}")

        #print('before_actor_pro',self.actor_model(**batch, use_cache=False))
        actor_prob = self.actor_model(**batch, use_cache=False).logits
        # print('actor_prob',actor_prob)
        actor_log_prob = gather_log_probs(actor_prob[:, :-1, :], seq[:, 1:])
        # print('actor_log_prob',actor_log_prob)
        # print('start',start)
        # print('actor_log_prob_start',actor_log_prob[:, start:])
        # print('log_probs_start',log_probs[:, start:])
        ...
        # print('value_start',value[:, start:])
        # print('old_value_start',old_values[:, start:])
